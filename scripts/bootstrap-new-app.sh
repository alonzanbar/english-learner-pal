...
# Prereqs: gh CLI installed and authenticated (gh auth login). Run from repo root (lexicon-learner-pal or any repo that has scripts/bootstrap-template/).
...
# Target directory: parent of current repo, so we don't create inside this repo
...
# Fix README clone URL: replace YOUR_USERNAME with actual gh username
